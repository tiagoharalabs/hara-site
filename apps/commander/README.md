...
The customer-visible bootstrap commands also fail closed on redirects: Linux requires HTTPS/TLS 1.2+ and zero redirects before piping to `bash`; Windows uses `-MaximumRedirection 0` before piping to `iex`. Both customer-facing commands explicitly pin `HARA_COMMANDER_URL=https://commander.haralabs.com.br` for the bootstrap execution so a stale inherited override cannot redirect enrollment; the Windows command restores the caller's previous environment value afterward. This narrows the initial web-origin trust surface but does not create an independent trust anchor.
